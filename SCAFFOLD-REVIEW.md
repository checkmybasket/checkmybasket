...
- **Follow-up (11 July):** prediction-round creation locked to **organiser-only** per product decision — migration `20260711103024` (INSERT policy now `is_group_organiser(group_id) AND status='open'`), and the "Start the game" button in the predictions tab is hidden from non-organisers (they see a "waiting for the organiser" note). Verified: organiser creates, member blocked.

**Update 11 July (code-quality pass, commit `32ab2a7`):** N1 ✅ duplicated `GiftCard` extracted to `components/gift-card.tsx` (`size="sm"|"lg"` variant; verified `/gifts` renders compact "View" cards and `/gifts/[category]` renders "Buy now" cards, no crossover). N4 ✅ `timeAgo()` clamps future timestamps and adds w/mo/y units. N3 ✅ fixed two real small-text gold-on-white contrast failures (anon-message sender label → `--cmb-primary`; "Wishlist" badge → new `--cmb-gold-strong` `#8A6D2F` @ 4.87:1); gold-on-dark-green left as-is. Still open: N2 (inline-styles→Tailwind, large refactor — scope it), the 2 low-severity security fixes (wishlist membership checks + `get_group_preview` rate limit), email notifications (Hassan's Resend decision), domain cutover (Hassan-triggered). Full continuation detail in `NEXT-SESSION-BRIEF.md`.

**Update 12 July (N2 complete — inline styles → Tailwind, 16/16 files):** finished the last 4 pages of the N2 refactor, completing the whole item. Static `style={{}}` colour/font/shadow/flex props were replaced with `[var(--cmb-*)]` arbitrary-value utilities (+ `font-display`, `shadow-[var(--shadow-N)]`, `border`/`border-2 border-dashed`, `bg-white`, `text-white`, `shrink-0`, `mt-0.5`) that resolve to identical CSS — **zero intended visual change**, verified page-by-page. Per-page inline-style counts and commits: `app/join/[invite_code]/page.tsx` 39→4 (`12492c6`), `app/create/page.tsx` 38→9 (`81085b9`), `app/g/[group_id]/settings/page.tsx` 34→5 (`785b788`), `app/g/[group_id]/page.tsx` 148→20 (`b9074ac`, the big one — done in sections with a build between each). The counts that remain are the deliberate leave-inline set only: `rgba()` literals, `backdropFilter`, the WhatsApp `#25D366` hex (no token), the `color-mix` priority tint, the hsl-based `Avatar`, and every dynamic/conditional value. Non-token hex literals were treated like rgba (left inline) except the canonical `#fff`→`bg-white`/`text-white` mapping. Verification: `npx next build` green after every page; served build screenshot-checked against a **live 3-member drawn group** (set up via anon signups + service-SQL, then drawn through the real UI) covering no-access, pre-draw organiser (progress/members/exclusions/draw-confirm), wishlists (incl. dashed empty state), post-draw match card + privacy + gifts-bought, settings organiser view, and the Games intro — all pixel-faithful. All 4 deploys reached READY on prod; all verification test data deleted (`groups`/`group_members`/`draws`/`auth.users` confirmed back to 0). Gotcha recorded for next time: the `@supabase/ssr` browser client session does not survive the browser-pane tool's hard `navigate()`, so authed pages must be reached via one continuous in-app flow using the app's own client-side `<Link>`s, and a post-insert refresh is forced by client-side navigating away and back (remount re-runs `refresh()`).

**Update 11 July (security features — all 3 done):** the outstanding low-severity items are applied to prod DB and mirrored into `supabase/migrations/`.
- `20260711214435_tighten_wishlist_claims_insert_membership` — `wishlist_claims` INSERT `WITH CHECK` now also requires `is_group_member(item.group_id)` and blocks self-claim (`wi.user_id <> auth.uid()`).
- `20260711214443_tighten_wishlist_items_write_membership` — `wishlist_items` "Owner manages own wishlist" (FOR ALL): reads/deletes stay ownership-only (`USING user_id = auth.uid()`), but INSERT/UPDATE now require `WITH CHECK (user_id = auth.uid() AND is_group_member(group_id))`.
- `20260711214459_rate_limit_get_group_preview` — rewrote the anon-callable preview as VOLATILE plpgsql with an IP-keyed limit (60 calls / IP / 10 min) via new `rl_group_preview` table (RLS on, 0 policies → SECURITY-DEFINER-only). Fail-open when `x-forwarded-for` is absent. Note: one upsert write per anon preview call (small, IP-keyed); table grows one row per distinct IP with no auto-prune yet — add a pg_cron sweep if it ever matters.
- **Verified** via RLS attacker simulation (self-rolling-back `DO` block, no residue): attacker blocked from inserting items/claims into a non-member group, legit member insert still works, owner self-claim blocked; rate limiter trips on exactly the 61st call and fails open with no header. `EXECUTE` grants on `get_group_preview` preserved for anon/authenticated. Prod DB confirmed clean (0 rows) afterward.

**Update 11 July (email notifications — scaffold built, activates on key):** optional post-reveal email capture + draw notification.
- `profiles.email` (pre-existing, no client grant) stays behind two SECURITY DEFINER own-row RPCs `set_my_email(text)` / `get_my_email()` (migration `20260711221929`) — co-member SELECT policy can't leak emails.
- Edge function `send-draw-emails` (deployed, `verify_jwt=true`, `supabase/functions/send-draw-emails/`): organiser-gated + drawn-gated, reads emails via service role, sends via Resend, **no-op when `RESEND_API_KEY` unset**. E2E-verified against prod with real anon sessions (organiser 200-skipped, recipient join eligible=1, outsider 403, no-auth 401, bad-input 400); test data cleaned.
- Client: `handleDraw` fire-and-forget invokes the fn post-`execute_draw`; `EmailCapture` card on the reveal screen. `tsconfig.json` excludes `supabase/functions` (Deno) from the Next build. **Blocked on Hassan:** add `RESEND_API_KEY` secret + verify a Resend sending domain (then set `EMAIL_FROM`), add a privacy-policy line.

**Update 11 July (domain cutover — LIVE):** `www.checkmybasket.co.uk` is now the primary custom domain (HTTP 200, valid TLS, serving the app); apex `checkmybasket.co.uk` 308-redirects → www. DNS points at Vercel (apex A `216.198.79.1` — Vercel's newer anycast IP, not the old `76.76.21.21`; www → `*.vercel-dns-017.com`). Verified live: canonical + og:url + og:image + twitter tags all resolve to the www host and the OG image renders (200 image/png ~60KB), so WhatsApp/Twitter link previews now work (were broken pre-cutover, by design of `metadataBase`). `NEXT_PUBLIC_APP_URL` resolving correctly in prod — no redeploy required.
