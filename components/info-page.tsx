  ...
  return (
    <div className="min-h-dvh flex flex-col" style={{ background: "var(--cmb-bg)" }}>
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-10">
        <h1 className="text-3xl font-bold mb-2" style={{ fontFamily: "var(--font-fraunces)" }}>{title}</h1>
        {updated && <p className="text-sm mb-8" style={{ color: "var(--cmb-text-muted)" }}>Last updated: {updated}</p>}
        <div className="info-prose space-y-6">{children}</div>
      </main>
      <footer className="py-8 px-4 border-t text-center text-xs" style={{ borderColor: "var(--cmb-border)", color: "var(--cmb-text-muted)" }}>
...
export function Section({ heading, children }: { heading: string; children: React.ReactNode }) {
  return (
    <section>
      <h2 className="text-lg font-semibold mb-2" style={{ fontFamily: "var(--font-fraunces)" }}>{heading}</h2>
      <div className="space-y-3 text-sm leading-relaxed" style={{ color: "var(--cmb-text-secondary)" }}>{children}</div>
    </section>
  );
}
