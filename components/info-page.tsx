  ...
  return (
    <div className="min-h-dvh flex flex-col" style={{ background: "var(--gc-bg)" }}>
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-10">
        ...
        {updated && <p className="text-sm mb-8" style={{ color: "var(--gc-text-muted)" }}>Last updated: {updated}</p>}
        <div className="info-prose space-y-6">{children}</div>
      </main>
      <footer className="py-8 px-4 border-t text-center text-xs" style={{ borderColor: "var(--gc-border)", color: "var(--gc-text-muted)" }}>
...
export function Section({ heading, children }: { heading: string; children: React.ReactNode }) {
  return (
    <section>
      ...
      <div className="space-y-3 text-sm leading-relaxed" style={{ color: "var(--gc-text-secondary)" }}>{children}</div>
    </section>
  );
}
