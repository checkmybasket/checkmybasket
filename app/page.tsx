...
export default function HomePage() {
  return (
    <div className="flex flex-col min-h-dvh" style={{ background: "var(--cmb-bg)" }}>
      {/* Nav */}
      <header className="sticky top-0 z-40 border-b" style={{ borderColor: "var(--cmb-border)", background: "rgba(255,248,240,0.92)", backdropFilter: "blur(12px)" }}>
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <nav className="hidden sm:flex items-center gap-6 text-sm" style={{ color: "var(--cmb-text-secondary)" }}>
          ...
          <Link href="/create">
            <Button size="sm" className="h-9 px-4 rounded-lg text-sm font-semibold" style={{ background: "var(--cmb-primary)", color: "var(--cmb-text-inverse)" }}>
              Create a free draw
            </Button>
          </Link>
        </div>
      ...
      <section className="noise-bg relative overflow-hidden" style={{ background: "var(--cmb-primary)" }}>
        <div className="relative z-10 max-w-5xl mx-auto px-4 py-20 sm:py-28 text-center">
          <div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-sm mb-6 animate-fade-in"
            style={{ background: "rgba(255,248,240,0.1)", color: "var(--cmb-warm)", border: "1px solid rgba(212,165,116,0.3)" }}>
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold mb-5 animate-fade-up"
            style={{ color: "var(--cmb-text-inverse)", fontFamily: "var(--font-fraunces)", lineHeight: 1.15 }}>
          ...
          <p className="text-lg sm:text-xl max-w-xl mx-auto mb-8 animate-fade-up animate-delay-100"
            ...
            <strong style={{ color: "var(--cmb-warm)" }}>No ads, ever.</strong>
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center animate-fade-up animate-delay-200">
            <Link href="/create">
              <Button size="lg" className="w-full sm:w-auto h-12 px-8 text-base rounded-xl font-semibold"
                style={{ background: "var(--cmb-accent)", color: "#fff" }}>
            ...
            <Link href="/gifts">
              <Button size="lg" variant="outline" className="w-full sm:w-auto h-12 px-8 text-base rounded-xl"
                style={{ borderColor: "rgba(255,248,240,0.3)", color: "var(--cmb-text-inverse)", background: "transparent" }}>
                View gift ideas
              </Button>
            </Link>
          </div>
      ...
      <section id="how-it-works" className="py-16 sm:py-24 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-sm font-medium mb-2" style={{ color: "var(--cmb-accent)" }}>How it works</p>
      ...
      {/* Features */}
      <section className="py-16 sm:py-24 px-4" style={{ background: "var(--cmb-surface)" }}>
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-sm font-medium mb-2" style={{ color: "var(--cmb-accent)" }}>Everything included</p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map(({ icon: Icon, title, body }) => (
              <div key={title} className="rounded-2xl p-6" style={{ background: "var(--cmb-bg)", border: "1px solid var(--cmb-border)" }}>
                <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-4" style={{ background: "var(--cmb-primary)", color: "var(--cmb-text-inverse)" }}>
                  <Icon size={20} strokeWidth={1.5} />
                </div>
                <h3 className="font-semibold text-base mb-1.5">{title}</h3>
                <p className="text-sm leading-relaxed" style={{ color: "var(--cmb-text-secondary)" }}>{body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 sm:py-20 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            ...
            <p style={{ color: "var(--cmb-text-secondary)" }}>Curated gifts from UK shops, filtered by budget. No ads.</p>
          </div>
          <div className="flex flex-wrap gap-3 justify-center">
            {budgetLinks.map((l) => (
              <Link key={l.href} href={l.href}
                ...
                style={{ background: "var(--cmb-surface)", border: "1px solid var(--cmb-border)", color: "var(--cmb-text-primary)", boxShadow: "var(--shadow-sm)" }}>
      ...
      {/* Bottom CTA */}
      <section className="py-16 px-4" style={{ background: "var(--cmb-primary)" }}>
        <div className="max-w-2xl mx-auto text-center">
          <p className="text-sm font-medium mb-3" style={{ color: "var(--cmb-warm)" }}>Secret Santa, sorted.</p>
          <h2 className="text-3xl sm:text-4xl font-bold mb-4" style={{ fontFamily: "var(--font-fraunces)", color: "var(--cmb-text-inverse)" }}>
          ...
          <div className="flex flex-wrap gap-4 justify-center mb-8">
            {["Free forever","No account needed","Works on any phone"].map((t) => (
              <div key={t} className="flex items-center gap-2 text-sm" style={{ color: "rgba(255,248,240,0.8)" }}>
                <CheckCircle2 size={16} strokeWidth={1.5} style={{ color: "var(--cmb-warm)" }} /> {t}
              </div>
            ))}
          </div>
          <Link href="/create">
            <Button size="lg" className="h-14 px-10 text-base rounded-xl font-semibold" style={{ background: "var(--cmb-accent)", color: "#fff" }}>
              Create a free draw <ArrowRight size={18} strokeWidth={1.5} className="ml-2" />
            </Button>
          </Link>
        </div>
      ...
      <footer className="py-10 px-4 border-t" style={{ borderColor: "var(--cmb-border)", background: "var(--cmb-surface)" }}>
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-sm" style={{ color: "var(--cmb-text-muted)" }}>
          <div className="flex flex-col sm:flex-row items-center gap-2">
            <div className="flex items-center gap-2">
              <Gift size={16} strokeWidth={1.5} style={{ color: "var(--cmb-primary)" }} />
              <span style={{ fontFamily: "var(--font-fraunces)", color: "var(--cmb-primary)", fontWeight: 600 }}>CheckMyBasket</span>
            </div>
        ...
        <div className="max-w-5xl mx-auto mt-5 pt-4 border-t flex flex-col sm:flex-row items-center justify-between gap-3 text-xs"
          style={{ borderColor: "var(--cmb-border)", color: "var(--cmb-text-muted)" }}>
