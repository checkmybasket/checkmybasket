      ...
      <header className="sticky top-0 z-30 border-b" style={{ borderColor:"var(--gc-border)", background:"rgba(255,248,240,0.92)", backdropFilter:"blur(12px)" }}>
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <span className="font-semibold truncate" style={{ fontFamily:"var(--font-fraunces)", color:"var(--gc-primary)" }}>{cat.label}</span>
          </div>
          <Link href="/create"><Button size="sm" className="h-9 px-3 rounded-lg text-xs font-semibold flex-shrink-0" style={{ background:"var(--gc-primary)", color:"var(--gc-text-inverse)" }}>Create draw</Button></Link>
        </div>
      </header>
      <main className="max-w-5xl mx-auto px-4 py-10">
        <div className="mb-8">
          <h1 className="text-3xl sm:text-4xl font-bold mb-2" style={{ fontFamily:"var(--font-fraunces)" }}>{cat.label}</h1>
          <p style={{ color:"var(--gc-text-secondary)" }}>{cat.desc}</p>
        </div>
        <div className="flex gap-2 overflow-x-auto pb-2 mb-8">
          {Object.entries(CATEGORIES).filter(([s])=>s!==category).map(([slug,c]) => (
            <Link key={slug} href={`/gifts/${slug}`}
              ...
              style={{ background:"var(--gc-surface)", borderColor:"var(--gc-border)", color:"var(--gc-text-secondary)" }}>{c.label}</Link>
          ))}
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
          {products.map(item => <GiftCard key={item.title} {...item}/>)}
        </div>
        <div className="rounded-2xl p-8 text-center mb-8" style={{ background:"var(--gc-primary)", boxShadow:"var(--shadow-lg)" }}>
          <h2 className="text-2xl font-bold mb-2" style={{ fontFamily:"var(--font-fraunces)", color:"var(--gc-text-inverse)" }}>Found the perfect gift?</h2>
          ...
          <Link href="/create"><Button size="lg" className="h-12 px-8 rounded-xl font-semibold" style={{ background:"var(--gc-accent)", color:"#fff" }}>Create a free draw</Button></Link>
        </div>
        <div className="rounded-xl p-4 flex gap-2" style={{ background:"var(--gc-surface)", border:"1px solid var(--gc-border)" }}>
          ...
          <p className="text-xs" style={{ color:"var(--gc-text-muted)" }}>Some gift links may earn us a small commission at no extra cost to you. This is how we keep CheckMyBasket free and ad-free.</p>
        </div>
      </main>
    </div>
  );
...
function GiftCard({ title, price, shop, tags, url }: { title:string; price:number; shop:string; tags:string[]; url:string }) {
  return (
    <div className="rounded-2xl overflow-hidden transition-shadow duration-200 hover:shadow-lg"
      ...
      <div className="w-full h-40 flex items-center justify-center" style={{ background:"var(--gc-surface-hover)" }}>
        <Gift size={40} strokeWidth={1} style={{ color:"var(--gc-border)" }}/>
      </div>
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mt-3">
          <div><p className="text-lg font-bold" style={{ color:"var(--gc-primary)" }}>£{(price/100).toFixed(0)}</p><p className="text-xs" style={{ color:"var(--gc-text-muted)" }}>{shop}</p></div>
          <a href={url} target="_blank" rel="noopener sponsored">
            <Button className="h-10 px-4 rounded-xl text-sm font-semibold" style={{ background:"var(--gc-accent)", color:"#fff" }}>
