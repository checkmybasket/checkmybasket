...
export default function JoinPage({ params }: { params: Promise<{ invite_code: string }> }) {
  ...
  if (loadState === "notfound" || !group) return (
    <div className="min-h-dvh flex flex-col items-center justify-center px-4 text-center" style={{ background:"var(--cmb-bg)" }}>
      <Gift size={40} strokeWidth={1.5} className="mb-4" style={{ color:"var(--cmb-text-muted)" }} />
    ...
    <div className="min-h-dvh" style={{ background:"var(--cmb-bg)" }}>
      <div className="max-w-md mx-auto px-4 py-8 pb-28 space-y-5">
        <div className="rounded-2xl p-4 flex items-center gap-3 animate-scale-in"
          ...
          <div className="w-10 h-10 rounded-full flex items-center justify-center text-lg" style={{ background:"var(--cmb-primary)", color:"var(--cmb-text-inverse)" }}>🎉</div>
          <div>
            <p className="font-semibold text-sm">You&apos;re in, {name}!</p>
            <p className="text-xs" style={{ color:"var(--cmb-text-secondary)" }}>While you wait for the draw, help your Secret Santa out:</p>
          </div>
        </div>
        {[
          ...
          <div key={title} className="rounded-2xl p-5 animate-fade-up" style={{ background:"var(--cmb-surface)", border:"1px solid var(--cmb-border)", boxShadow:"var(--shadow-sm)" }}>
            <h2 className="font-semibold mb-1">{title}</h2>
            <p className="text-sm mb-3" style={{ color:"var(--cmb-text-muted)" }}>{hint}</p>
            <Textarea placeholder={ph} value={val} onChange={e => set(e.target.value)} className="min-h-[72px] rounded-xl text-base resize-none" style={{ borderColor:"var(--cmb-border-strong)" }}/>
          </div>
        ))}
        <div className="rounded-2xl p-5 animate-fade-up animate-delay-200" style={{ background:"var(--cmb-surface)", border:"1px solid var(--cmb-border)", boxShadow:"var(--shadow-sm)" }}>
          <h2 className="font-semibold mb-1">Wishlist items</h2>
          <p className="text-sm mb-3" style={{ color:"var(--cmb-text-muted)" }}>Add links from any shop</p>
          <div className="space-y-2">
            ...
            <Button variant="ghost" onClick={() => setWishItems([...wishItems,""])} className="h-10 text-sm rounded-xl w-full" style={{ color:"var(--cmb-primary)" }}>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-20 px-4 pt-4 border-t safe-bottom" style={{ background:"var(--cmb-bg)", borderColor:"var(--cmb-border)" }}>
        <div className="max-w-md mx-auto flex gap-3">
          <Button variant="outline" size="lg" disabled={busy} onClick={() => router.push(`/g/${group.group_id}`)} className="h-12 flex-1 rounded-xl" style={{ borderColor:"var(--cmb-border-strong)" }}>
            Skip for now
          </Button>
          <Button size="lg" disabled={busy} onClick={saveOnboarding} className="h-12 flex-1 rounded-xl font-semibold" style={{ background:"var(--cmb-primary)", color:"var(--cmb-text-inverse)" }}>
    ...
    <div className="min-h-dvh flex flex-col" style={{ background:"var(--cmb-bg)" }}>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-12">
        <div className="w-full max-w-md space-y-5">
          <div className="rounded-2xl p-6 animate-scale-in" style={{ background:"var(--cmb-surface)", border:"1px solid var(--cmb-border)", boxShadow:"var(--shadow-lg)" }}>
            <div className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium mb-4"
              style={{ background:"rgba(27,67,50,0.08)", color:"var(--cmb-primary)" }}>
            ...
            {group.organiser_name && <p className="text-sm mb-4" style={{ color:"var(--cmb-text-secondary)" }}>Created by {group.organiser_name}</p>}
            <div className="flex gap-4 flex-wrap">
              <div className="flex items-center gap-1.5 text-sm" style={{ color:"var(--cmb-text-secondary)" }}>
                <Users size={14} strokeWidth={1.5}/> {group.member_count} {group.member_count === 1 ? "person" : "people"} joined
              </div>
              {group.budget_amount != null && <div className="text-sm font-medium" style={{ color:"var(--cmb-primary)" }}>Budget: £{group.budget_amount/100}</div>}
              {exchangeDate && <div className="text-sm" style={{ color:"var(--cmb-text-secondary)" }}>{exchangeDate}</div>}
            </div>
          </div>
          <div className="rounded-2xl p-6 animate-fade-up" style={{ background:"var(--cmb-surface)", border:"1px solid var(--cmb-border)", boxShadow:"var(--shadow-md)" }}>
            ...
            {group.draw_status !== "pending" ? (
              <p className="text-sm" style={{ color:"var(--cmb-text-secondary)" }}>
                ...
                <Link href={`/g/${group.group_id}`} className="underline" style={{ color:"var(--cmb-primary)" }}>open your group</Link>.
              </p>
            ) : (
              <>
                <div className="mb-5">
                  ...
                  {nameError && <p className="mt-1 text-sm" style={{ color:"var(--cmb-error)" }}>{nameError}</p>}
                </div>
                <Button onClick={handleJoin} disabled={busy} size="lg" className="w-full h-12 text-base rounded-xl font-semibold" style={{ background:"var(--cmb-primary)", color:"var(--cmb-text-inverse)" }}>
                  {busy ? "Joining…" : <>Join this Secret Santa <ArrowRight size={18} strokeWidth={1.5} className="ml-2"/></>}
                </Button>
                <p className="text-center text-xs mt-3" style={{ color:"var(--cmb-text-muted)" }}>No account needed. Joining is free.</p>
              </>
            )}
          </div>
        </div>
